 ...
 */
class SearchCharacteristicsFragment : Fragment() {
    ...
    private fun characteristicsItemClickHandler(view: View) {
        ...
        if (tableRow[view.id % steps].tag == NON_SELECTED_BUBBLE) {
            ...
            (activity as ChatSearchActivity).chosenCharacteristics.add(characteristics[view.id])
        } else {
            bubbleFrame = LayoutInflater.from(activity).inflate(
                R.layout.user_profile_bubble_blue_selected,
                null
            ...
            (activity as ChatSearchActivity).chosenCharacteristics.remove(characteristics[view.id])
        }
