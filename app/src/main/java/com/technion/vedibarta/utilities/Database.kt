...
class Database {
    ...
    inner class StoragePathBuilder(private val userId: String) {
        private var path = ""
    ...
    private class DataBasePathBuilder(
    ...
    private fun uploadFile(bytes: ByteArray, path: String): StorageTask<UploadTask.TaskSnapshot> {
        val filePathRef = storage.child(path)
        return filePathRef.putBytes(bytes)
    }
    ...
    fun uploadProfilePicture(photo: Uri): StorageTask<UploadTask.TaskSnapshot>? {
        ...
        if (userId != null) {
            val path =
                StoragePathBuilder(userId).students().userId().pictures().fileName("profile_pic")
    ...
    fun uploadProfilePicture(bytes: ByteArray): StorageTask<UploadTask.TaskSnapshot>? {
        var task: StorageTask<UploadTask.TaskSnapshot>? = null
        if (userId != null) {
            val path =
                StoragePathBuilder(userId).students().userId().pictures().fileName("profile_pic")
            logStart(Action.UPLOAD_PROFILE_PICTURE)
            task = uploadFile(bytes, path)
                .addOnSuccessListener {
                    logSuccess(Action.UPLOAD_PROFILE_PICTURE)
                }
                .addOnFailureListener { e: Exception ->
                    logFailure(
                        e,
